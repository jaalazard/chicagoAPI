...
*/
  ...
  /* useEffect(() => {
    const fetchData = () => {
      fetch("https://api.artic.edu/api/v1/artworks/search?q=picasso")
        ...
        .then((data) => {
          setArt(data.data); // Je mets le résultat de ma requête dans "art" via setArt;
  ...
  return (
    <>
    {console.log(art)}
      {Array.from(art).map((artwork, index) => (
        <div key={index}>
          <h1>{`${artwork.title}, par ${artwork.classification_titles[0]}`}</h1>
        <p>{`Référence de l'oeuvre : ${artwork.id}`}</p>
        <h2>{`Description : ${artwork.thumbnail.alt_text}`}</h2>
        </div>
      ))}
    </>
  );
